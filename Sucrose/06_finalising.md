...
1. The next stage is to label your structure to a sensible numbering scheme. In `@Toolbox-Work|Labels` select **Labels On/Off** until the atom labels are displayed (alternatively press **F3**). Hide the Q-peaks either using the **Q** square in `@Toolbox-Work` or use CTRL+Q.
...
4.	Now with the left hand mouse button click on the atoms in a sensible order to name them sequentially from your chosen start number. The atom labels of the atoms that have been clicked on change from green to black.
