...
8.	Carry out further refinement cycles until all atoms are approximately represented by spheres of the same size.
9.	At this stage using olex2.refine you should have R1 ~ 8-9%, wR2 ~25%, Difference map max ~0.7 e– Å-3, min ~-0.6 e Å-3. 
10.	The next stage is to model the atoms using anisotropic displacement parameters (ADP) i.e. using 6 parameters to define the volume occupied by the electron density rather than the single one that was used to define the isotropic displacement parameter.
...
12.	Again the atoms should be roughly similar size and shape in similar environments. If you have incorrect atom assignments at this stage tetrahedrally-shaped atoms will indicate atoms that are non-positive definite and should be heavier (i.e. O not C) while large ellipsoids indicate the atom type should be lighter (i.e. C not O). In the illustration: atoms 3 are correctly assigned atom types with similar sized spheres, 1 is non-positive definite (in this case too light atom type) and 2 is a large ellipsoid, i.e. assigned as too heavy atom type.
